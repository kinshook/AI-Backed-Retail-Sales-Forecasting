...
def create_retail_summary(sales_df, adjust_df, engine):
   ...
   retail_summary['True_Profit'] = (
        retail_summary['Revenue'] + retail_summary['Charges_Pounds']  if retail_summary['Charges_Pounds']<0 else retail_summary['Revenue'] - retail_summary['Charges_Pounds']
    )
...
if __name__=="__main__":
    ...
    logger.info(f"Connection established and {len(df.keys())} tables {list(df.keys())} fetched in {end-st:.2f} seconds.")
  
    merged_df = merge_tables(df)
    sales_df, adjust_df = merged_table_Cleaning(merged_df) #Shape:    (5620, 14)
